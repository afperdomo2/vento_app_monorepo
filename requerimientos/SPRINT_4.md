...
- [ ] **Alertas** (⛔ NO SE IMPLEMENTARÁN):
  - ⛔ DLQ growing — Requiere configuración de alerting rules en Grafana/Prometheus que presentó problemas de estabilidad en pruebas locales
  - ⛔ High error rate — Mismo motivo; se recomienda implementar en Sprint 6 con solución de alerting dedicada (PagerDuty, OpsGenie)
  - ⛔ Low available tickets — Alerta de negocio que depende de thresholds configurables por evento; fuera del scope de observabilidad técnica
...
- [x] Alertas configuradas para DLQ y error rate (⛔ no implementadas — ver sección 8.3)
- [x] Logs incluyen traceId para correlación
- [x] Health checks funcionan para todos los componentes
- [x] Endpoint Prometheus protegido en producción (solo health+info)
- [x] Prometheus + Grafana + OTEL + Jaeger en docker-compose.dev.yml
- [x] Prometheus + Grafana + OTEL + Jaeger en docker-compose.prod.yml
...
- **Semana 8.2**: Métricas con Micrometer + Prometheus (100%)
  - ✅ Métricas HTTP y JVM automáticas
  - ✅ 9 métricas custom implementadas
  - ✅ Prometheus en docker-compose.local.yml, docker-compose.dev.yml, docker-compose.prod.yml
  - ✅ Endpoint protegido en producción (solo health+info expuestos)
- **Semana 8.3**: Grafana Dashboards (100%)
  ...
  - ⛔ Alertas: NO implementadas — problemas de estabilidad con alerting rules; recomendado para Sprint 6
- **Semana 8.4**: Logging Centralizado (100%)
  ...
  - ✅ Datasource Loki separado en `provisioning-dev/` (no se carga en local)
- **Semana 8.5**: Health Checks y Readiness (100%)
  ...
  - ℹ️ Liveness/Readiness: no aplica para Docker Compose, configuración disponible para futura migración a Kubernetes

### ⛔ Decisiones del Sprint:
- **Alertas (Grafana/Prometheus)**: No se implementaron debido a problemas de estabilidad con alerting rules en pruebas locales. Se recomienda implementar en Sprint 6 con solución dedicada (PagerDuty, OpsGenie).
...
- **Prometheus + Grafana + OTEL + Jaeger en dev y prod** ✅
- **Endpoints protegidos en producción** ✅

**Stack Pendiente:**
- Ninguno — Sprint 4 completado
- ⛔ Alertas: diferidas a Sprint 6 (ver sección 8.3)

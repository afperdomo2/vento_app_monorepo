...
- [x] Endpoint en API Gateway para routing a ambos servicios

### 2.5 - Tests Unitarios

- [x] Tests para `EventService`:
    - Crear evento
    - Verificar decremento de availableTickets
    - Manejo de capacidad agotada
- [x] Tests para `OrderService`:
    - Crear orden exitosa
    - Rechazar cuando no hay tickets
    - Cancelar orden
- [x] Tests para `EventClient` (mock)
...
Semana 2 ──────────────────────────────────────
  ...
  2.2 Order models     ✅ Completado (Order, OrderRepository, OrderService, OrderController)
  2.3 OpenFeign        ✅ Completado (EventClient configurado)
  2.4 Flujo E2E        ✅ Completado (crear evento → reservar entrada)
  2.5 Tests            ✅ Completado (16 tests: 6 EventService + 10 OrderService)
```
...
- [x] Keycloak accesible en http://localhost:8180 con realm `vento-realm`
- [x] Dashboard de Keycloak permite crear/editar/eliminar usuarios
- [x] Solicitudes sin token JWT retornan 401 Unauthorized
- [x] Solicitudes con token válido pasan a través del Gateway con header `X-User-Id`
- [x] POST /api/events crea un evento y guarda en PostgreSQL
- [x] POST /api/orders crea una reserva verificando disponibilidad en event-service
- [x] GET /api/events retorna lista paginada con documentación Swagger
- [x] Tests unitarios pasan con `./gradlew test`

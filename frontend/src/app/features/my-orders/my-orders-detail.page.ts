...
import { OrderStatus, EnrichedOrder, OrderDetailState, initialOrderDetailState } from './models/order.model';
          ...
          <div class="space-y-6">
            ...
            <div class="relative rounded-2xl overflow-hidden shadow-lg">
              ...
              <div class="absolute bottom-4 left-4 right-4">
                <h2 class="text-white font-headline text-xl font-bold">{{ order()!.eventTitle }}</h2>
            ...
            <div class="bg-surface-container rounded-2xl p-6 shadow-sm">
              <h3 class="font-headline text-lg font-bold text-on-surface mb-4">Estado del Pedido</h3>
              <div class="flex items-center justify-between">
                ...
                <div class="flex flex-col items-center gap-2">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center"
                       ...
                       [class.bg-primary-container]="order()?.status !== 'PENDING' && order()?.status !== 'CANCELLED' && order()?.status !== 'EXPIRED'"
                       [class.bg-surface-container-high]="order()?.status === 'CANCELLED' || order()?.status === 'EXPIRED'">
                    <span class="material-symbols-outlined text-sm"
                          [class.text-white]="order()?.status === 'PENDING'"
                          [class.text-primary]="order()?.status !== 'PENDING' && order()?.status !== 'CANCELLED' && order()?.status !== 'EXPIRED'"
                          [class.text-on-surface-variant]="order()?.status === 'CANCELLED' || order()?.status === 'EXPIRED'">
                ...
                <div class="flex-1 h-0.5 mx-2"
                     ...
                     [class.bg-surface-container-high]="order()?.status !== 'CONFIRMED'"></div>
                <div class="flex flex-col items-center gap-2">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center"
                       [class.bg-primary-container]="order()?.status === 'CONFIRMED'"
                       [class.bg-error-container]="order()?.status === 'CANCELLED' || order()?.status === 'EXPIRED'"
                       [class.bg-surface-container-high]="order()?.status === 'PENDING'">
                    <span class="material-symbols-outlined text-sm"
                          [class.text-primary]="order()?.status === 'CONFIRMED'"
                          [class.text-error]="order()?.status === 'CANCELLED' || order()?.status === 'EXPIRED'"
                          [class.text-on-surface-variant]="order()?.status === 'PENDING'">
                      {{ order()?.status === 'CONFIRMED' ? 'check_circle' : order()?.status === 'CANCELLED' || order()?.status === 'EXPIRED' ? 'cancel' : 'hourglass_empty' }}
                    </span>
                  </div>
                  <span class="text-xs font-semibold"
                        [class.text-primary]="order()?.status === 'CONFIRMED'"
            ...
            <div class="bg-surface-container rounded-2xl shadow-sm overflow-hidden">
              ...
              <div class="p-6 space-y-4">
                <div class="flex justify-between items-center">
                  ...
                  <span class="font-mono text-sm text-on-surface">{{ formatOrderId(order()!.id) }}</span>
                </div>
                <div class="flex justify-between items-center">
                ...
                <div class="border-t border-outline-variant/10 pt-4 flex justify-between items-center">
                  ...
                  <span class="font-headline text-xl font-bold text-primary">{{ formatTotal(order()!.totalAmount) }}</span>
                </div>
                <div class="flex justify-between items-center text-xs">
                  <span class="text-on-surface-variant">Creado</span>
                  <span class="text-on-surface-variant">{{ formatDateTime(order()!.createdAt) }}</span>
                </div>
                @if (order()!.updatedAt !== order()!.createdAt) {
                  <div class="flex justify-between items-center text-xs">
                    <span class="text-on-surface-variant">Última actualización</span>
                    <span class="text-on-surface-variant">{{ formatDateTime(order()!.updatedAt) }}</span>
                  </div>
                }
              </div>
            </div>
